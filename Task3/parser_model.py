...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        selected_embeddings = torch.index_select(self.embeddings, 0, flattened_w)
        x = selected_embeddings.view(w.size(0), -1)
    ...
    def forward(self, w):
        ...
        embedding_result = self.embedding_lookup(w)
        hidden_layer_input = torch.matmul(embedding_result, self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        hidden_layer_output = F.relu(hidden_layer_input)
        dropout_output = self.dropout(hidden_layer_output)
        logits = torch.matmul(dropout_output, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
